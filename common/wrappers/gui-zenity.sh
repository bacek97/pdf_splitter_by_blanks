...
choose_folder() {
  zenity --file-selection --directory --title="$MSG_INLINE_CHOOSE_FOLDER"
}

set_template() {
  zenity --entry --title="$MSG_INLINE_SET_TMPL_NAME" --text="$MSG_INLINE_SET_TMPL_NAME" --entry-text "$FILENAME_EVERY_FILE"
}
